...
export class DashboardService {
  ...
  public get(): Observable<Array<Dashboard>> {
    return this._client.get<{ dashboards: Array<Dashboard> }>(`${this._baseUrl}api/dashboard`)
      .pipe(
        map(x => x.dashboards)
      );
  }
  ...
  public create(options: { dashboard: Dashboard }): Observable<{ dashboardId: string  }> {    
    return this._client.post<{ dashboardId: string }>(`${this._baseUrl}api/dashboard`, { dashboard: options.dashboard });
  }
